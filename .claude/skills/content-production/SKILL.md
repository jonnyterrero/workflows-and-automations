...
Work in `automations/content-engine/runs/<YYYY-MM-DD>/<slug>/`, alongside the draft that produced it. Create `assets/`. Record the source draft path in the manifest's `source_draft` field so the render is traceable to its script.
