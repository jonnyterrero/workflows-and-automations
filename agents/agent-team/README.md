...
**Every new Claude Code session already loads the team skills** — the `project`
copy lives in `.claude/skills/`, is committed, and Claude Code discovers it on
open. A `SessionStart` hook (`.claude/helpers/sync-agent-skills.cjs`) re-copies
`agents/agent-team/skills/` over that directory at start, so editing the canonical
...
After `sync_cursor_export.py`, run `install_cursor_local.py` on each machine (uses `%USERPROFILE%\.cursor` / `$HOME/.cursor` — no hardcoded user paths). Or open this repo as a workspace and point Cursor at `agents/agent-team/cursor/`.
